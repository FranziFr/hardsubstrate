...
# time between midpoints of time bins (Ordovician stages)
t <- c(9.65,7.7,5.2,5.8,7.15,6.6,4.6)
# time per interval/stageslice (Ordovician stages)
...
for (i in 1:100){
  ...
  tryCatch({
    ...
    ## to assign a genus, we use the genus names that we extracted above, before the loop started
      ...
      genus.name %in% Trilobita.names ~ "Trilo",
      genus.name %in% Mollusca.names ~ "Mollusc"
...
for (i in 1:100){
  ...
  ## after running the model, we collect the results from the model in the list reruns
  reruns.Trilobites[[i]]=time.1$results$real
  
}

reruns.lambda.Trilobites=list()
...
for (i in 1:100){
  ...
  reruns.lambda.Trilobites[[i]]=time.lambda$results$real
  
}

reruns.Brachiopods=list()
...
for (i in 1:100){
  ...
  ## after running the model, we collect the results from the model in the list reruns
  reruns.Brachiopods[[i]]=time.1$results$real
  
}

reruns.lambda.Brachiopods=list()
...
for (i in 1:100){
  ...
  reruns.lambda.Brachiopods[[i]]=time.lambda$results$real
  
}

reruns.Molluscs=list()
...
for (i in 1:100){
  ...
  ## after running the model, we collect the results from the model in the list reruns
  reruns.Molluscs[[i]]=time.1$results$real
  
}

reruns.lambda.Molluscs=list()
...
for (i in 1:100){
  ...
  reruns.lambda.Molluscs[[i]]=time.lambda$results$real
...
###
## collecting 100 replicate parameter estimates
estimates.Trilos <- do.call(cbind, lapply(reruns.Trilobites, function(x) x$estimate))
lcl.Trilos <- do.call(cbind, lapply(reruns.Trilobites, function(x) x$lcl))
ucl.Trilos <- do.call(cbind, lapply(reruns.Trilobites, function(x) x$ucl))


estimates.Trilos[estimates.Trilos<0.00001|estimates.Trilos==1|estimates.Trilos>=1] <- NA
lcl.Trilos[is.na(estimates.Trilos)] <- NA
ucl.Trilos[is.na(estimates.Trilos)] <- NA

estimates.Trilos[ucl.Trilos-lcl.Trilos==0|ucl.Trilos-lcl.Trilos==1|ucl.Trilos>1]<-NA
lcl.Trilos[is.na(estimates.Trilos)] <- NA
ucl.Trilos[is.na(estimates.Trilos)] <- NA

##
estimates.mean.Trilos <- rowMeans(estimates.Trilos, na.rm = T)
lcl.mean.Trilos <- rowMeans(lcl.Trilos, na.rm = T)
ucl.mean.Trilos <- rowMeans(ucl.Trilos, na.rm = T)

estimates.median.Trilos <- apply(estimates.Trilos, 1, median, na.rm=TRUE)
lcl.median.Trilos <- apply(lcl.Trilos,1,median, na.rm=TRUE)
ucl.median.Trilos <- apply(ucl.Trilos,1,median, na.rm=TRUE)

##
##
estimates.lambda.Trilos <- do.call(cbind, lapply(reruns.lambda.Trilobites, function(x) x$estimate))
lcl.lambda.Trilos <- do.call(cbind, lapply(reruns.lambda.Trilobites, function(x) x$lcl))
ucl.lambda.Trilos <- do.call(cbind, lapply(reruns.lambda.Trilobites, function(x) x$ucl))

# estimates.lambda[estimates.lambda<0.00001|estimates.lambda==1|estimates.lambda>=1] <- NA
# lcl.lambda[is.na(estimates.lambda)] <- NA
# ucl.lambda[is.na(estimates.lambda)] <- NA
# 
estimates.lambda.Trilos[ucl.lambda.Trilos-lcl.lambda.Trilos==0|ucl.lambda.Trilos-lcl.lambda.Trilos>=15]<-NA
lcl.lambda.Trilos[is.na(estimates.lambda.Trilos)] <- NA
ucl.lambda.Trilos[is.na(estimates.lambda.Trilos)] <- NA

estimates.lambda.mean.Trilos <- rowMeans(estimates.lambda.Trilos, na.rm = T)
lcl.lambda.mean.Trilos <- rowMeans(lcl.lambda.Trilos, na.rm = T)
ucl.lambda.mean.Trilos <- rowMeans(ucl.lambda.Trilos, na.rm = T)

estimates.lambda.median.Trilos <- apply(estimates.lambda.Trilos, 1, median, na.rm=TRUE)
lcl.lambda.median.Trilos <- apply(lcl.lambda.Trilos, 1, median, na.rm=TRUE)
ucl.lambda.median.Trilos <- apply(ucl.lambda.Trilos, 1, median, na.rm=TRUE)

###########################################################################################
## extracting parameter estimates from the results above
phi.Trilos <- estimates.Trilos[2:8,]
phiu.Trilos <- ucl.Trilos[2:8,]
phil.Trilos <- lcl.Trilos[2:8,]

p.Trilos <- estimates.Trilos[13:19,]
pu.Trilos <- ucl.Trilos[13:19,]
pl.Trilos <- lcl.Trilos[13:19,]

gam.Trilos <- estimates.Trilos[23:29,]
gamu.Trilos <- ucl.Trilos[23:29,]
gaml.Trilos <- lcl.Trilos[23:29,]


lambda.Trilos <- estimates.lambda.Trilos[23:29,]
lambdau.Trilos <- ucl.lambda.Trilos[23:29,]
lambdal.Trilos <- lcl.lambda.Trilos[23:29,]

## extracting parameter estimates medians from the results above
phi.median.Trilos <- estimates.median.Trilos[2:8]
phiu.median.Trilos <- ucl.median.Trilos[2:8]
phil.median.Trilos <- lcl.median.Trilos[2:8]

p.median.Trilos <- estimates.median.Trilos[13:19]
pu.median.Trilos <- ucl.median.Trilos[13:19]
pl.median.Trilos <- lcl.median.Trilos[13:19]

gam.median.Trilos <- estimates.median.Trilos[23:29]
gamu.median.Trilos <- ucl.median.Trilos[23:29]
gaml.median.Trilos <- lcl.median.Trilos[23:29]


lambda.median.Trilos <- estimates.lambda.median.Trilos[23:29]
lambdau.median.Trilos <- ucl.lambda.median.Trilos[23:29]
lambdal.median.Trilos <- lcl.lambda.median.Trilos[23:29]


origprob.Trilos <- 1-gam.Trilos
origCIl.Trilos <- 1-gaml.Trilos
origCIu.Trilos <- 1-gamu.Trilos
Orig_rate.Trilos <- -log(1-origprob.Trilos)/t
Orig_rate_CIl.Trilos <- -log(1-origCIl.Trilos)/t
Orig_rate_CIu.Trilos <- -log(1-origCIu.Trilos)/t

extinctprob.Trilos <- 1-phi.Trilos
extinctCIl.Trilos <- 1-phil.Trilos
extinctCIu.Trilos <- 1-phiu.Trilos
Ext_rate.Trilos <- -log(1-extinctprob.Trilos)/t
Ext_rate_CIl.Trilos <- -log(1-extinctCIl.Trilos)/t
Ext_rate_CIu.Trilos <- -log(1-extinctCIu.Trilos)/t

rate_p.Trilos <- -log(1-p.Trilos)/tp
ratel_p.Trilos <- -log(1-pl.Trilos)/tp
rateu_p.Trilos <- -log(1-pu.Trilos)/tp

### estimating median rates
origprob.median.Trilos <- 1-gam.median.Trilos
origCIl.median.Trilos <- 1-gaml.median.Trilos
origCIu.median.Trilos <- 1-gamu.median.Trilos
Orig_rate.median.Trilos <- -log(1-origprob.median.Trilos)/t
Orig_rate_CIl.median.Trilos <- -log(1-origCIl.median.Trilos)/t
Orig_rate_CIu.median.Trilos <- -log(1-origCIu.median.Trilos)/t

extinctprob.median.Trilos <- 1-phi.median.Trilos
extinctCIl.median.Trilos <- 1-phil.median.Trilos
extinctCIu.median.Trilos <- 1-phiu.median.Trilos
Ext_rate.median.Trilos <- -log(1-extinctprob.median.Trilos)/t
Ext_rate_CIl.median.Trilos <- -log(1-extinctCIl.median.Trilos)/t
Ext_rate_CIu.median.Trilos <- -log(1-extinctCIu.median.Trilos)/t

rate_p.median.Trilos <- -log(1-p.median.Trilos)/tp
ratel_p.median.Trilos <- -log(1-pl.median.Trilos)/tp
rateu_p.median.Trilos <- -log(1-pu.median.Trilos)/tp
###
###
###
## collecting 100 replicate parameter estimates
estimates.Brachios <- do.call(cbind, lapply(reruns.Brachiopods, function(x) x$estimate))
lcl.Brachios <- do.call(cbind, lapply(reruns.Brachiopods, function(x) x$lcl))
ucl.Brachios <- do.call(cbind, lapply(reruns.Brachiopods, function(x) x$ucl))


estimates.Brachios[estimates.Brachios<0.00001|estimates.Brachios==1|estimates.Brachios>=1] <- NA
lcl.Brachios[is.na(estimates.Brachios)] <- NA
ucl.Brachios[is.na(estimates.Brachios)] <- NA

estimates.Brachios[ucl.Brachios-lcl.Brachios==0|ucl.Brachios-lcl.Brachios==1|ucl.Brachios>1]<-NA
lcl.Brachios[is.na(estimates.Brachios)] <- NA
ucl.Brachios[is.na(estimates.Brachios)] <- NA

##
estimates.mean.Brachios <- rowMeans(estimates.Brachios, na.rm = T)
lcl.mean.Brachios <- rowMeans(lcl.Brachios, na.rm = T)
ucl.mean.Brachios <- rowMeans(ucl.Brachios, na.rm = T)

estimates.median.Brachios <- apply(estimates.Brachios, 1, median, na.rm=TRUE)
lcl.median.Brachios <- apply(lcl.Brachios,1,median, na.rm=TRUE)
ucl.median.Brachios <- apply(ucl.Brachios,1,median, na.rm=TRUE)

##
##
estimates.lambda.Brachios <- do.call(cbind, lapply(reruns.lambda.Brachiopods, function(x) x$estimate))
lcl.lambda.Brachios <- do.call(cbind, lapply(reruns.lambda.Brachiopods, function(x) x$lcl))
ucl.lambda.Brachios <- do.call(cbind, lapply(reruns.lambda.Brachiopods, function(x) x$ucl))

# estimates.lambda[estimates.lambda<0.00001|estimates.lambda==1|estimates.lambda>=1] <- NA
# lcl.lambda[is.na(estimates.lambda)] <- NA
# ucl.lambda[is.na(estimates.lambda)] <- NA
# 
estimates.lambda.Brachios[ucl.lambda.Brachios-lcl.lambda.Brachios==0|ucl.lambda.Brachios-lcl.lambda.Brachios>=15]<-NA
lcl.lambda.Brachios[is.na(estimates.lambda.Brachios)] <- NA
ucl.lambda.Brachios[is.na(estimates.lambda.Brachios)] <- NA

estimates.lambda.mean.Brachios <- rowMeans(estimates.lambda.Brachios, na.rm = T)
lcl.lambda.mean.Brachios <- rowMeans(lcl.lambda.Brachios, na.rm = T)
ucl.lambda.mean.Brachios <- rowMeans(ucl.lambda.Brachios, na.rm = T)

estimates.lambda.median.Brachios <- apply(estimates.lambda.Brachios, 1, median, na.rm=TRUE)
lcl.lambda.median.Brachios <- apply(lcl.lambda.Brachios, 1, median, na.rm=TRUE)
ucl.lambda.median.Brachios <- apply(ucl.lambda.Brachios, 1, median, na.rm=TRUE)

###########################################################################################
## extracting parameter estimates from the results above
phi.Brachios <- estimates.Brachios[2:8,]
phiu.Brachios <- ucl.Brachios[2:8,]
phil.Brachios <- lcl.Brachios[2:8,]

p.Brachios <- estimates.Brachios[13:19,]
pu.Brachios <- ucl.Brachios[13:19,]
pl.Brachios <- lcl.Brachios[13:19,]

gam.Brachios <- estimates.Brachios[23:29,]
gamu.Brachios <- ucl.Brachios[23:29,]
gaml.Brachios <- lcl.Brachios[23:29,]


lambda.Brachios <- estimates.lambda.Brachios[23:29,]
lambdau.Brachios <- ucl.lambda.Brachios[23:29,]
lambdal.Brachios <- lcl.lambda.Brachios[23:29,]

## extracting parameter estimates medians from the results above
phi.median.Brachios <- estimates.median.Brachios[2:8]
phiu.median.Brachios <- ucl.median.Brachios[2:8]
phil.median.Brachios <- lcl.median.Brachios[2:8]

p.median.Brachios <- estimates.median.Brachios[13:19]
pu.median.Brachios <- ucl.median.Brachios[13:19]
pl.median.Brachios <- lcl.median.Brachios[13:19]

gam.median.Brachios <- estimates.median.Brachios[23:29]
gamu.median.Brachios <- ucl.median.Brachios[23:29]
gaml.median.Brachios <- lcl.median.Brachios[23:29]


lambda.median.Brachios <- estimates.lambda.median.Brachios[23:29]
lambdau.median.Brachios <- ucl.lambda.median.Brachios[23:29]
lambdal.median.Brachios <- lcl.lambda.median.Brachios[23:29]


origprob.Brachios <- 1-gam.Brachios
origCIl.Brachios <- 1-gaml.Brachios
origCIu.Brachios <- 1-gamu.Brachios
Orig_rate.Brachios <- -log(1-origprob.Brachios)/t
Orig_rate_CIl.Brachios <- -log(1-origCIl.Brachios)/t
Orig_rate_CIu.Brachios <- -log(1-origCIu.Brachios)/t

extinctprob.Brachios <- 1-phi.Brachios
extinctCIl.Brachios <- 1-phil.Brachios
extinctCIu.Brachios <- 1-phiu.Brachios
Ext_rate.Brachios <- -log(1-extinctprob.Brachios)/t
Ext_rate_CIl.Brachios <- -log(1-extinctCIl.Brachios)/t
Ext_rate_CIu.Brachios <- -log(1-extinctCIu.Brachios)/t

rate_p.Brachios <- -log(1-p.Brachios)/tp
ratel_p.Brachios <- -log(1-pl.Brachios)/tp
rateu_p.Brachios <- -log(1-pu.Brachios)/tp

### estimating median rates
origprob.median.Brachios <- 1-gam.median.Brachios
origCIl.median.Brachios <- 1-gaml.median.Brachios
origCIu.median.Brachios <- 1-gamu.median.Brachios
Orig_rate.median.Brachios <- -log(1-origprob.median.Brachios)/t
Orig_rate_CIl.median.Brachios <- -log(1-origCIl.median.Brachios)/t
Orig_rate_CIu.median.Brachios <- -log(1-origCIu.median.Brachios)/t

extinctprob.median.Brachios <- 1-phi.median.Brachios
extinctCIl.median.Brachios <- 1-phil.median.Brachios
extinctCIu.median.Brachios <- 1-phiu.median.Brachios
Ext_rate.median.Brachios <- -log(1-extinctprob.median.Brachios)/t
Ext_rate_CIl.median.Brachios <- -log(1-extinctCIl.median.Brachios)/t
Ext_rate_CIu.median.Brachios <- -log(1-extinctCIu.median.Brachios)/t

rate_p.median.Brachios <- -log(1-p.median.Brachios)/tp
ratel_p.median.Brachios <- -log(1-pl.median.Brachios)/tp
rateu_p.median.Brachios <- -log(1-pu.median.Brachios)/tp
###
###
###
## collecting 100 replicate parameter estimates
estimates.Molluscs <- do.call(cbind, lapply(reruns.Molluscs, function(x) x$estimate))
lcl.Molluscs <- do.call(cbind, lapply(reruns.Molluscs, function(x) x$lcl))
ucl.Molluscs <- do.call(cbind, lapply(reruns.Molluscs, function(x) x$ucl))


estimates.Molluscs[estimates.Molluscs<0.00001|estimates.Molluscs==1|estimates.Molluscs>=1] <- NA
lcl.Molluscs[is.na(estimates.Molluscs)] <- NA
ucl.Molluscs[is.na(estimates.Molluscs)] <- NA

estimates.Molluscs[ucl.Molluscs-lcl.Molluscs==0|ucl.Molluscs-lcl.Molluscs==1|ucl.Molluscs>1]<-NA
lcl.Molluscs[is.na(estimates.Molluscs)] <- NA
ucl.Molluscs[is.na(estimates.Molluscs)] <- NA

##
estimates.mean.Molluscs <- rowMeans(estimates.Molluscs, na.rm = T)
lcl.mean.Molluscs <- rowMeans(lcl.Molluscs, na.rm = T)
ucl.mean.Molluscs <- rowMeans(ucl.Molluscs, na.rm = T)

estimates.median.Molluscs <- apply(estimates.Molluscs, 1, median, na.rm=TRUE)
lcl.median.Molluscs <- apply(lcl.Molluscs,1,median, na.rm=TRUE)
ucl.median.Molluscs <- apply(ucl.Molluscs,1,median, na.rm=TRUE)

##
##
estimates.lambda.Molluscs <- do.call(cbind, lapply(reruns.lambda.Molluscs, function(x) x$estimate))
lcl.lambda.Molluscs <- do.call(cbind, lapply(reruns.lambda.Molluscs, function(x) x$lcl))
ucl.lambda.Molluscs <- do.call(cbind, lapply(reruns.lambda.Molluscs, function(x) x$ucl))

# estimates.lambda[estimates.lambda<0.00001|estimates.lambda==1|estimates.lambda>=1] <- NA
# lcl.lambda[is.na(estimates.lambda)] <- NA
# ucl.lambda[is.na(estimates.lambda)] <- NA
# 
estimates.lambda.Molluscs[ucl.lambda.Molluscs-lcl.lambda.Molluscs==0|ucl.lambda.Molluscs-lcl.lambda.Molluscs>=15]<-NA
lcl.lambda.Molluscs[is.na(estimates.lambda.Molluscs)] <- NA
ucl.lambda.Molluscs[is.na(estimates.lambda.Molluscs)] <- NA

estimates.lambda.mean.Molluscs <- rowMeans(estimates.lambda.Molluscs, na.rm = T)
lcl.lambda.mean.Molluscs <- rowMeans(lcl.lambda.Molluscs, na.rm = T)
ucl.lambda.mean.Molluscs <- rowMeans(ucl.lambda.Molluscs, na.rm = T)

estimates.lambda.median.Molluscs <- apply(estimates.lambda.Molluscs, 1, median, na.rm=TRUE)
lcl.lambda.median.Molluscs <- apply(lcl.lambda.Molluscs, 1, median, na.rm=TRUE)
ucl.lambda.median.Molluscs <- apply(ucl.lambda.Molluscs, 1, median, na.rm=TRUE)

###########################################################################################
## extracting parameter estimates from the results above
phi.Molluscs <- estimates.Molluscs[2:8,]
phiu.Molluscs <- ucl.Molluscs[2:8,]
phil.Molluscs <- lcl.Molluscs[2:8,]

p.Molluscs <- estimates.Molluscs[13:19,]
pu.Molluscs <- ucl.Molluscs[13:19,]
pl.Molluscs <- lcl.Molluscs[13:19,]

gam.Molluscs <- estimates.Molluscs[23:29,]
gamu.Molluscs <- ucl.Molluscs[23:29,]
gaml.Molluscs <- lcl.Molluscs[23:29,]


lambda.Molluscs <- estimates.lambda.Molluscs[23:29,]
lambdau.Molluscs <- ucl.lambda.Molluscs[23:29,]
lambdal.Molluscs <- lcl.lambda.Molluscs[23:29,]

## extracting parameter estimates medians from the results above
phi.median.Molluscs <- estimates.median.Molluscs[2:8]
phiu.median.Molluscs <- ucl.median.Molluscs[2:8]
phil.median.Molluscs <- lcl.median.Molluscs[2:8]

p.median.Molluscs <- estimates.median.Molluscs[13:19]
pu.median.Molluscs <- ucl.median.Molluscs[13:19]
pl.median.Molluscs <- lcl.median.Molluscs[13:19]

gam.median.Molluscs <- estimates.median.Molluscs[23:29]
gamu.median.Molluscs <- ucl.median.Molluscs[23:29]
gaml.median.Molluscs <- lcl.median.Molluscs[23:29]


lambda.median.Molluscs <- estimates.lambda.median.Molluscs[23:29]
lambdau.median.Molluscs <- ucl.lambda.median.Molluscs[23:29]
lambdal.median.Molluscs <- lcl.lambda.median.Molluscs[23:29]

origprob.Molluscs <- 1-gam.Molluscs
origCIl.Molluscs <- 1-gaml.Molluscs
origCIu.Molluscs <- 1-gamu.Molluscs
Orig_rate.Molluscs <- -log(1-origprob.Molluscs)/t
Orig_rate_CIl.Molluscs <- -log(1-origCIl.Molluscs)/t
Orig_rate_CIu.Molluscs <- -log(1-origCIu.Molluscs)/t

extinctprob.Molluscs <- 1-phi.Molluscs
extinctCIl.Molluscs <- 1-phil.Molluscs
extinctCIu.Molluscs <- 1-phiu.Molluscs
Ext_rate.Molluscs <- -log(1-extinctprob.Molluscs)/t
Ext_rate_CIl.Molluscs <- -log(1-extinctCIl.Molluscs)/t
Ext_rate_CIu.Molluscs <- -log(1-extinctCIu.Molluscs)/t

rate_p.Molluscs <- -log(1-p.Molluscs)/tp
ratel_p.Molluscs <- -log(1-pl.Molluscs)/tp
rateu_p.Molluscs <- -log(1-pu.Molluscs)/tp

### estimating median rates
origprob.median.Molluscs <- 1-gam.median.Molluscs
origCIl.median.Molluscs <- 1-gaml.median.Molluscs
origCIu.median.Molluscs <- 1-gamu.median.Molluscs
Orig_rate.median.Molluscs <- -log(1-origprob.median.Molluscs)/t
Orig_rate_CIl.median.Molluscs <- -log(1-origCIl.median.Molluscs)/t
Orig_rate_CIu.median.Molluscs <- -log(1-origCIu.median.Molluscs)/t

extinctprob.median.Molluscs <- 1-phi.median.Molluscs
extinctCIl.median.Molluscs <- 1-phil.median.Molluscs
extinctCIu.median.Molluscs <- 1-phiu.median.Molluscs
Ext_rate.median.Molluscs <- -log(1-extinctprob.median.Molluscs)/t
Ext_rate_CIl.median.Molluscs <- -log(1-extinctCIl.median.Molluscs)/t
Ext_rate_CIu.median.Molluscs <- -log(1-extinctCIu.median.Molluscs)/t

rate_p.median.Molluscs <- -log(1-p.median.Molluscs)/tp
ratel_p.median.Molluscs <- -log(1-pl.median.Molluscs)/tp
rateu_p.median.Molluscs <- -log(1-pu.median.Molluscs)/tp
###
###
###
...
###
###
save.image("//kant/nhm-sfs-u2/franzif/paper2/100.comp.Fig.RData")
load("//kant/nhm-sfs-u2/franzif/paper2/100.comp.Fig.RData")
...
arrows(x0=Stagebase, y0=Orig_rate_CIl.median.Echinos, x1=Stagebase, y1=Orig_rate_CIu.median.Echinos,length=0.02, lwd = 1.2, angle = 90, code = 3, lty=1, col = "black")

lines(Stagebase-0.2, Orig_rate.median.Bryozoa[1:7], col = "orange", type = "b")
arrows(x0=Stagebase-0.2, y0=Orig_rate_CIl.median.Bryozoa, x1=Stagebase-0.2, y1=Orig_rate_CIu.median.Bryozoa,length=0.02, lwd = 1.2, angle = 90, code = 3, lty=1, col = "orange")

lines(Stagebase+0.2, Orig_rate.median.Porifera[1:7], col = "skyblue", type = "b")
arrows(x0=Stagebase+0.2, y0=Orig_rate_CIl.median.Porifera, x1=Stagebase+0.2, y1=Orig_rate_CIu.median.Porifera,length=0.02, lwd = 1.2, angle = 90, code = 3, lty=1, col = "skyblue")

# lines(Stagebase, Orig_rate.median.Corals[1:7], col = "seagreen3", type = "b")
# arrows(x0=Stagebase, y0=Orig_rate_CIl.median.Corals, x1=Stagebase, y1=Orig_rate_CIu.median.Corals,length=0.02, lwd = 1.2, angle = 90, code = 3, lty=1, col = "seagreen3")

lines(Stagebase-0.4, Orig_rate.median.Trilos[1:7], col = "yellow", type = "b")
arrows(x0=Stagebase-0.4, y0=Orig_rate_CIl.median.Trilos, x1=Stagebase-0.4, y1=Orig_rate_CIu.median.Trilos,length=0.02, lwd = 1.2, angle = 90, code = 3, lty=1, col = "yellow")

lines(Stagebase+0.4, Orig_rate.median.Brachios[1:7], col = "midnightblue", type = "b")
arrows(x0=Stagebase+0.4, y0=Orig_rate_CIl.median.Brachios, x1=Stagebase+0.4, y1=Orig_rate_CIu.median.Brachios,length=0.02, lwd = 1.2, angle = 90, code = 3, lty=1, col = "midnightblue")

lines(Stagebase-0.6, Orig_rate.median.Molluscs[1:7], col = "deeppink", type = "b")
...
legend("topright",
       c("Echinodermata","Bryozoa","Porifera",
         #"Corals",
         "Trilobita","Brachiopoda","Mollusca"),
       inset=0.05,
       box.lty = 0,
       lty=1,
...
arrows(x0=Stagebase, y0=Ext_rate_CIl.median.Echinos, x1=Stagebase, y1=Ext_rate_CIu.median.Echinos,length=0.02, lwd = 1.2, angle = 90, code = 3, lty=1, col = "black")

lines(Stagebase-0.2, Ext_rate.median.Bryozoa[1:7], col = "orange", type = "b")
arrows(x0=Stagebase-0.2, y0=Ext_rate_CIl.median.Bryozoa, x1=Stagebase-0.2, y1=Ext_rate_CIu.median.Bryozoa,length=0.02, lwd = 1.2, angle = 90, code = 3, lty=1, col = "orange")

lines(Stagebase+0.2, Ext_rate.median.Porifera[1:7], col = "skyblue", type = "b")
arrows(x0=Stagebase+0.2, y0=Ext_rate_CIl.median.Porifera, x1=Stagebase+0.2, y1=Ext_rate_CIu.median.Porifera,length=0.02, lwd = 1.2, angle = 90, code = 3, lty=1, col = "skyblue")

# lines(Stagebase, Ext_rate.median.Corals[1:7], col = "seagreen3", type = "b")
# arrows(x0=Stagebase, y0=Ext_rate_CIl.median.Corals, x1=Stagebase, y1=Ext_rate_CIu.median.Corals,length=0.02, lwd = 1.2, angle = 90, code = 3, lty=1, col = "seagreen3")

lines(Stagebase-0.4, Ext_rate.median.Trilos[1:7], col = "yellow", type = "b")
arrows(x0=Stagebase-0.4, y0=Ext_rate_CIl.median.Trilos, x1=Stagebase-0.4, y1=Ext_rate_CIu.median.Trilos,length=0.02, lwd = 1.2, angle = 90, code = 3, lty=1, col = "yellow")

lines(Stagebase+0.4, Ext_rate.median.Brachios[1:7], col = "midnightblue", type = "b")
arrows(x0=Stagebase+0.4, y0=Ext_rate_CIl.median.Brachios, x1=Stagebase+0.4, y1=Ext_rate_CIu.median.Brachios,length=0.02, lwd = 1.2, angle = 90, code = 3, lty=1, col = "midnightblue")

lines(Stagebase-0.6, Ext_rate.median.Molluscs[1:7], col = "deeppink", type = "b")
arrows(x0=Stagebase-0.6, y0=Ext_rate_CIl.median.Molluscs, x1=Stagebase-0.6, y1=Ext_rate_CIu.median.Molluscs,length=0.02, lwd = 1.2, angle = 90, code = 3, lty=1, col = "deeppink")
...
arrows(x0=Stagebase, y0=lambdal.median.Echinos-1, x1=Stagebase, y1=lambdau.median.Echinos-1,length=0.02, lwd = 1.2, angle = 90, code = 3, lty=1, col = "black")

lines(Stagebase-0.2, lambda.median.Bryozoa[1:7]-1, col = "orange", type = "b")
arrows(x0=Stagebase-0.2, y0=lambdal.median.Bryozoa-1, x1=Stagebase-0.2, y1=lambdau.median.Bryozoa-1,length=0.02, lwd = 1.2, angle = 90, code = 3, lty=1, col = "orange")

lines(Stagebase+0.2, lambda.median.Porifera[1:7]-1, col = "skyblue", type = "b")
arrows(x0=Stagebase+0.2, y0=lambdal.median.Porifera-1, x1=Stagebase+0.2, y1=lambdau.median.Porifera-1,length=0.02, lwd = 1.2, angle = 90, code = 3, lty=1, col = "skyblue")

# lines(Stagebase, lambda.median.Corals[1:7]-1, col = "seagreen3", type = "b")
# arrows(x0=Stagebase, y0=lambdal.median.Corals-1, x1=Stagebase, y1=lambdau.median.Corals-1,length=0.02, lwd = 1.2, angle = 90, code = 3, lty=1, col = "seagreen3")

lines(Stagebase-0.4, lambda.median.Trilos[1:7]-1, col = "yellow", type = "b")
arrows(x0=Stagebase-0.4, y0=lambdal.median.Trilos-1, x1=Stagebase-0.4, y1=lambdau.median.Trilos-1,length=0.02, lwd = 1.2, angle = 90, code = 3, lty=1, col = "yellow")

lines(Stagebase+0.4, lambda.median.Brachios[1:7]-1, col = "midnightblue", type = "b")
arrows(x0=Stagebase+0.4, y0=lambdal.median.Brachios-1, x1=Stagebase+0.4, y1=lambdau.median.Brachios-1,length=0.02, lwd = 1.2, angle = 90, code = 3, lty=1, col = "midnightblue")

lines(Stagebase-0.6, lambda.median.Molluscs[1:7]-1, col = "deeppink", type = "b")
arrows(x0=Stagebase-0.6, y0=lambdal.median.Molluscs-1, x1=Stagebase-0.6, y1=lambdau.median.Molluscs-1,length=0.02, lwd = 1.2, angle = 90, code = 3, lty=1, col = "deeppink")
...
plot(Stagemidpoints, rate_p.median.Echinos[1:7],
     ...
     xlab = "",
     ylab = "")
tscales.Ord(1.7, -0.05, -0.3)


lines(Stagemidpoints, rate_p.median.Echinos[1:7], col = "black", type = "b")
arrows(x0=Stagemidpoints, y0=ratel_p.median.Echinos, x1=Stagemidpoints, y1=rateu_p.median.Echinos,length=0.02, lwd = 1.2, angle = 90, code = 3, lty=1, col = "black")

lines(Stagemidpoints-0.2, rate_p.median.Bryozoa[1:7], col = "orange", type = "b")
arrows(x0=Stagemidpoints-0.2, y0=ratel_p.median.Bryozoa, x1=Stagemidpoints-0.2, y1=rateu_p.median.Bryozoa,length=0.02, lwd = 1.2, angle = 90, code = 3, lty=1, col = "orange")

lines(Stagemidpoints+0.2, rate_p.median.Porifera[1:7], col = "skyblue", type = "b")
arrows(x0=Stagemidpoints+0.2, y0=ratel_p.median.Porifera, x1=Stagemidpoints+0.2, y1=rateu_p.median.Porifera,length=0.02, lwd = 1.2, angle = 90, code = 3, lty=1, col = "skyblue")

# lines(Stagemidpoints, rate_p.median.Corals[1:7], col = "seagreen3", type = "b")
# arrows(x0=Stagemidpoints, y0=ratel_p.median.Corals, x1=Stagemidpoints, y1=rateu_p.median.Corals,length=0.02, lwd = 1.2, angle = 90, code = 3, lty=1, col = "seagreen3")

lines(Stagemidpoints-0.4, rate_p.median.Trilos[1:7], col = "yellow", type = "b")
arrows(x0=Stagemidpoints-0.4, y0=ratel_p.median.Trilos, x1=Stagemidpoints-0.4, y1=rateu_p.median.Trilos,length=0.02, lwd = 1.2, angle = 90, code = 3, lty=1, col = "yellow")

lines(Stagemidpoints+0.4, rate_p.median.Brachios[1:7], col = "midnightblue", type = "b")
arrows(x0=Stagemidpoints+0.4, y0=ratel_p.median.Brachios, x1=Stagemidpoints+0.4, y1=rateu_p.median.Brachios,length=0.02, lwd = 1.2, angle = 90, code = 3, lty=1, col = "midnightblue")

lines(Stagemidpoints-0.6, rate_p.median.Molluscs[1:7], col = "deeppink", type = "b")
arrows(x0=Stagemidpoints-0.6, y0=ratel_p.median.Molluscs, x1=Stagemidpoints-0.6, y1=rateu_p.median.Molluscs,length=0.02, lwd = 1.2, angle = 90, code = 3, lty=1, col = "deeppink")
...
axis(2, col = 'grey75', line = -0.2, at = seq(0, 1.7, 0.2))
